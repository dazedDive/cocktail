...
const Content = ({drinks}) => {
    return (
        ...
        <div className="container w-1/2">
            ...
            {drinks.ingredients?.map(item=>{
                return(
                    <>
                    <div key={item?.idIngredient}>
                        <h1 className="mt-8 font-bodyFont text-5xl text-lime-500">{item.strIngredient}</h1>
                        <span className="flex flex-row">
                            <h3 className="text-slate-800 text-2xl mx-1">{item?.strType}  </h3>
                            <h3 className="text-lime-500 text-2xl mx-1">{item?.strAlcohol==="Yes" ? "Alcohol" : "no Alcohol"}</h3>
                        </span>
                        <p className="mt-3 text-white text-justify text-xl">{item?.strDescription}</p>
                    </div>
                    </>
                )
            })}
